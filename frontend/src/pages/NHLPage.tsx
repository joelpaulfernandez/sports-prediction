...
import { getNHLGames, getNHLAccuracy } from '../api/nhl';
import { NHLGameCard } from '../components/NHLGameCard';
import { NHLAccuracyBanner } from '../components/NHLAccuracyBanner';
...
function HeroAccuracyStat({ accuracy }: { accuracy: NHLAccuracyStats | null }) {
  if (!accuracy) {
    return (
      <div style={{ textAlign: 'right' }}>
        <div style={{
          fontFamily: 'var(--font-display)', fontSize: '72px', fontWeight: 900,
          letterSpacing: '-0.02em', color: 'var(--border-bright)', lineHeight: 1,
        }}>—</div>
        <div style={{
          fontFamily: 'var(--font-mono)', fontSize: '10px',
          color: 'var(--text-muted)', letterSpacing: '0.1em', marginTop: '8px',
        }}>NO DATA YET</div>
      </div>
    );
  }

  const hasLive = accuracy.total_predictions > 0;
  const pct = hasLive ? accuracy.accuracy_percentage : accuracy.backtest_accuracy * 100;
  ...
  return (
    <div style={{ textAlign: 'right', animation: 'fadeIn 0.6s ease both', animationDelay: '0.2s' }}>
      <div style={{
        fontFamily: 'var(--font-mono)', fontSize: '10px',
        color: 'var(--text-muted)', letterSpacing: '0.12em', marginBottom: '8px',
      }}>
        {hasLive ? 'SEASON ACCURACY' : 'BACKTESTED ACCURACY'}
      </div>
      <div style={{
      ...
      <div style={{
        fontFamily: 'var(--font-mono)', fontSize: '11px',
        color: 'var(--text-secondary)', marginTop: '10px', letterSpacing: '0.02em',
      }}>
        {hasLive ? (
          <>
          ...
          <>
            <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{accuracy.backtest_games.toLocaleString()}</span>
            {' '}games backtested
          </>
        )}
      </div>
...
export function NHLPage() {
  ...
  const [accuracy, setAccuracy] = useState<NHLAccuracyStats | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError(null);
        const [gamesData, accuracyData] = await Promise.all([getNHLGames(), getNHLAccuracy()]);
        if (!cancelled) {
          setGames(gamesData);
  ...
  // Auto-refresh every 60s when games are live
  useEffect(() => {
    const hasLive = games.some(g => g.status === 'live');
    if (!hasLive) return;
    const id = setInterval(() => {
      Promise.all([getNHLGames(), getNHLAccuracy()])
    ...
    <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px 96px', flex: 1 }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          gap: '48px', padding: '56px 0 52px', alignItems: 'center',
        }}>
          <div style={{ animation: 'fadeUp 0.5s ease both' }}>
            <div style={{
              ...
              <span style={{
                fontFamily: 'var(--font-mono)', fontSize: '10px',
                fontWeight: 600, color: 'var(--primary)', letterSpacing: '0.1em',
              }}>
                NHL · 2025–26 SEASON
              </span>
            </div>
      ...
      {!loading && !error && accuracy && (
        <div style={{ paddingTop: '40px' }}>
          <NHLAccuracyBanner stats={accuracy} />
        </div>
      )}
      ...
      {!loading && error && (
        <div style={{
          margin: '40px 0', background: 'var(--error-dim)',
